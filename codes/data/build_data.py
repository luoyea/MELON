...
def load_textual_image_features(data_path):
    ...
    text_feat_path = os.path.join(data_path, 'text_feat.npy')
    image_feat_path = os.path.join(data_path, 'image_feat.npy')
    if os.path.exists(text_feat_path) and os.path.exists(image_feat_path):
        print("Found existing text_feat.npy and image_feat.npy, skip Doc2Vec/Image conversion.")
        return
    ...
    text_vec = {}
    # gensim 3.x compatibility: use docvecs first.
    # gensim 4.x compatibility: use dv if available.
    doc_store = None
    if hasattr(doc2vec_model, 'dv'):
        doc_store = doc2vec_model.dv
    elif hasattr(doc2vec_model, 'docvecs'):
        doc_store = doc2vec_model.docvecs

    if doc_store is None:
        raise RuntimeError("Cannot find doc vectors in Doc2Vec model. Please use gensim==3.8.3 for this dataset.")

    for asin in asin_dict:
        text_vec[asin] = doc_store[asin]
