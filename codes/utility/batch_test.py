...
def test_torch(user_ice, item_ice, user_mce, item_mce, img_query, txt_query, uv_agg, ut_agg, image_feats, text_feats, v_rel_mlp, t_rel_mlp, users_to_test, is_val, adj, alpha, beta, gamma, model=None):
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)), 'auc': 0.0}
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        for i_batch_id in range(n_item_batchs):
            ...
            rel_score_tensor = torch.tensor(total_pos_rel_score_array, device=i_rate_batch.device, dtype=i_rate_batch.dtype)
            total_batch_score = i_rate_batch + beta * rel_score_tensor
            if model is not None and hasattr(model, 'enable_mq_component') and model.enable_mq_component:
                mq_score = model.compute_mq_score_for_eval(batch_user_ice, batch_item_ice, item_batch)
                total_batch_score = total_batch_score + model.eta_mq * mq_score
            rate_batch[:, i_start: i_end] = total_batch_score.detach().cpu().numpy()
